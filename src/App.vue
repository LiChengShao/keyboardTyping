...
export default {
  ...
  methods: {
    ...
    startDrag(e) {
      ...
      // e.clientX/clientY 是鼠标在视口的坐标，减去计时器的坐标得到「鼠标相对于计时器的点击位置」
    ...
    onDrag(e) {
      ...
      // 更新位置（使用 window 宽高计算百分比或直接使用像素）
      // 这里使用像素赋值最简单直接
      // e.clientX/clientY 是鼠标在视口的坐标，减去偏移量得到「鼠标相对于球体的点击位置」
...
.custom-timer-icon:hover {
  transform: translateY(-0%) scale(1.1);
  box-shadow: 0 8px 20px rgba(0, 0, 0, 0.4);
}

.custom-timer-icon .fas {
  font-size: 20rem;
  margin-bottom: 5px;
}
...
.todo-box {
  ...
  flex-direction: column;
  gap: 15px;
  z-index: 999; /* 确保在计时器图标之下，但仍在页面内容之上 */
  border: 1px solid rgba(255, 255, 255, 0.1);
}
